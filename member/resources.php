    ...
    // Fetch all unique subjects from modules (using the subject column directly)
    ...
    // Fetch all available modules with subject information
    $stmt = $conn->prepare("
        SELECT 
            m.id, 
            m.title, 
    ...
    <body class="bg-gray-50 text-gray-800 antialiased" x-data="{ 
        ...
        searchQuery: '', 
        selectedSubject: 'All',
        showAlert: <?= ($successMessage || $errorMessage) ? 'true' : 'false' ?> 
    }">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        ...
                        <div class="relative sm:w-64">
                            <select x-model="selectedSubject" 
                                    class="custom-select w-full px-4 py-2.5 sm:py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all text-sm sm:text-base bg-white">
                ...
                <div class="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
                    <?php foreach ($modules as $index => $module): ?>
                        <?php
                            ...
                            $subjectName = htmlspecialchars($module['subject_name'] ?? 'General');
                        ?>
                        <div x-show="(activeFilter === 'All' || activeFilter === '<?= htmlspecialchars($status) ?>') && 
                                    (selectedSubject === 'All' || selectedSubject === '<?= $subjectName ?>') &&
                            ...
                            <div class="h-32 xs:h-36 sm:h-40 overflow-hidden relative group">
                                ...
                                <div class="absolute bottom-2 sm:bottom-3 left-2 sm:left-3 right-2 sm:right-3">
                                    <div class="flex items-center justify-between gap-2 flex-wrap">
                                        ...
                                        <span class="inline-flex items-center px-2 sm:px-3 py-1 sm:py-1.5 rounded-full text-xs font-semibold bg-purple-100 text-purple-700 backdrop-blur-sm">
