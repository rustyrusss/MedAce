...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $conn->prepare("
            INSERT INTO quizzes (title, description, content, professor_id, lesson_id, status, created_at)
            VALUES (:title, :description, :content, :professor_id, :lesson_id, :status, NOW())
        ");
        $stmt->execute([
            ':title'       => $title,
